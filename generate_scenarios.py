...
def generate_scenarios(config, p, kmeans_model, transition_matrix, errors_,prediction_errors,solar_reg,qt,solar_capacity):
    ...
    for s in range(num_scenarios):
        ...
        simulated_data = pd.DataFrame(sample, index=pd.date_range(scenario_start_date, periods=periods * number_of_periods, freq="H", name="datetime"), columns=prediction_errors.columns)
        simulated_data.solar += solar_reg.predict(simulated_data.index)
        simulated_data = pd.DataFrame(qt.inverse_transform(simulated_data.values), index=simulated_data.index, columns=simulated_data.columns)
        simulated_data.solar = simulated_data.groupby(pd.Grouper(freq="D")).solar.transform(lambda x: x * solar_capacity[-1])

        scenarios[f'scenario_{s + 1}'] = simulated_data.solar
